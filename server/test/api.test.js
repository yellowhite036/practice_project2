...
function createMockPool(options = {}) {
  ...
  const pool = {
    async query(rawSql, params = []) {
      ...
      if (sql.includes("nextval")) return { rows: [{ seq: 12345 }] };
...
test("Work Order creation uses transaction and succeeds", async () => {
  ...
  assert.ok(res.body.work_order_id.startsWith("WO-"));
...
test("6. admin ?�以修改 materials", async () => {
  const app = createApp({ pool: createMockPool() });
...
test("7. manager ?�以修改 materials", async () => {
  const app = createApp({ pool: createMockPool() });
...
test("8. operator 修改 materials -> 403", async () => {
  const app = createApp({ pool: createMockPool() });
...
test("13. manager ?�以 reject work order", async () => {
  const app = createApp({ pool: createMockPool() });
